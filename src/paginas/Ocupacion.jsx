...
export default function Ocupacion() {
  ...
  const loadData = async () => {
    ...
    // 1. INTENTAR CARGAR ZONAS
    console.log("Cargando zonas...");
    const { data: zonasData, error: zonasError } = await supabase.from('ZONA_ESTACIONAMIENTO').select('*');
    
    if (zonasError) {
        alert("ERROR CARGANDO ZONAS: " + zonasError.message);
        console.error(zonasError);
    }
    setZonas(zonasData || []);

    // 2. INTENTAR CARGAR PLAZAS
    console.log("Cargando plazas...");
    // Nota: Quitamos temporalmente el JOIN complejo para ver si cargan las plazas solas primero
    const { data: plazasData, error: plazasError } = await supabase
      .from('PLAZA')
      .select(`
        *,
        ZONA_ESTACIONAMIENTO ( Nombre_Zona )
      `) 
      .order('Id_Plaza', { ascending: true });
    
    if (plazasError) {
        alert("ERROR CARGANDO PLAZAS: " + plazasError.message);
        console.error(plazasError);
    } else {
        console.log("Plazas cargadas:", plazasData?.length);
    }
    ...
    <Layout>
      <header className="mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-gray-500">Gestión visual del estacionamiento.</p>
        </div>
        <button 
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4">
            {filteredPlazas.length === 0 && (
                <div className="col-span-full text-center py-10 text-gray-500 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                   ...
                   <p className="text-xs mt-2">Si ves esto y hay datos en la BD, revisa la consola (F12) para ver errores.</p>
                </div>
            )}
            ...
            {filteredPlazas.map(plaza => (
                <div 
                    key={plaza.Id_Plaza}
                    className={`
                        relative group p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 shadow-sm
                        flex flex-col items-center justify-center text-center h-40
                        ${getCardColor(plaza.Estado_Actual)}
                    `}
                    onClick={() => toggleOccupancy(plaza)}
                >
                    <h3 className="font-bold text-lg text-gray-800">{plaza.Numero_Plaza}</h3>
                    <p className="text-xs text-gray-500 mt-1 mb-2 truncate w-full">
                        
                        {plaza.ZONA_ESTACIONAMIENTO?.Nombre_Zona || 'Sin Zona'}
                    </p>
                    
                    <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${getBadgeColor(plaza.Estado_Actual)}`}>
                        {plaza.Estado_Actual}
                    </span>
                    ...
                    <div className="absolute bottom-2 left-0 right-0 flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button 
                            ...
                            className={`p-1.5 rounded-full shadow transition 
                                ${plaza.Estado_Actual === 'Reservada' ? 'bg-yellow-500 text-white ring-2 ring-yellow-300' : 'bg-white hover:bg-yellow-100 text-yellow-600'}
                            `}
                            title="Reservar"
                        >
                            <FaClock size={12}/>
                        ...
                        <button 
                            ...
                            className={`p-1.5 rounded-full shadow transition 
                                ${plaza.Estado_Actual === 'Mantenimiento' ? 'bg-orange-500 text-white ring-2 ring-orange-300' : 'bg-white hover:bg-orange-100 text-orange-600'}
                            `}
                            title="Mantenimiento"
                        >
...
export default function Ocupacion() {
  ...
  useEffect(() => {
    ...
    // SUSCRIPCIÓN EN TIEMPO REAL (Para los Sensores)
    // Esto hará que si un sensor cambia el estado en la BD, la pantalla se actualice sola.
  ...
  const toggleOccupancy = (plaza) => {
    // Si es automático por sensor, el manual solo debería usarse para corregir o forzar
    if (plaza.Estado_Actual === 'Mantenimiento' || plaza.Estado_Actual === 'Reservada') return;
    const nuevo = plaza.Estado_Actual === 'Libre' ? 'Ocupado' : 'Libre';
    changeStatus(plaza.Id_Plaza, nuevo);
  };
